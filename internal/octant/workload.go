...
func NewClusterPodMetricsLoader(clusterClient cluster.ClientInterface) (*ClusterPodMetricsLoader, error) {
	if clusterClient == nil {
		return nil, fmt.Errorf("pod metrics loader is nil")
	}
	return &ClusterPodMetricsLoader{clusterClient: clusterClient}, nil
}

// Load loads metrics for a pod given namespace and a name.
func (ml *ClusterPodMetricsLoader) Load(namespace, name string) (*unstructured.Unstructured, error) {
	client, err := ml.clusterClient.DynamicClient()
	if err != nil {
		return nil, fmt.Errorf("get dynamic client: %w", err)
	}

	metricsRes := schema.GroupVersionResource{Group: "metrics.k8s.io", Version: "v1beta1", Resource: "pods"}

	options := metav1.GetOptions{}

	object, err := client.Resource(metricsRes).Namespace(namespace).Get(name, options)
	if err != nil {
...
func (wc *WorkloadCardCollector) Collect(ctx context.Context, namespace string) ([]*component.Card, error) {
	...
	for i := range workloads {
		...
		if err != nil {
			return nil, fmt.Errorf("create workload summary: %w", err)
		}

		memoryStat, err := PodMemoryStat(workload)
		if err != nil {
			return nil, fmt.Errorf("create workload memory chart: %w", err)
		}

		cpuStat, err := PodCPUStat(workload)
		if err != nil {
			return nil, fmt.Errorf("create workload cpu chart: %w", err)
		}
